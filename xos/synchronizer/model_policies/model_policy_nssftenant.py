...
class NSSFTenantPolicy(TenantWithContainerPolicy):
    model_name = "NSSFTenant"
    ...
    def handle_update(self, service_instance):
        if (service_instance.link_deleted_count>0) and (not service_instance.provided_links.exists()):
            ...
            if NSSFTenant.objects.filter(id=service_instance.id).exists():
    ...
    def handle_delete(self, service_instance):
        if service_instance.instance and (not service_instance.instance.deleted):
            all_service_instances_this_instance = NSSFTenant.objects.filter(instance_id=service_instance.instance.id)
            ...
            if (not other_service_instances_this_instance):
                self.logger.info("NSSFTenant Instance %s is now unused -- deleting" % service_instance.instance)
                self.delete_instance(service_instance, service_instance.instance)
            else:
                self.logger.info("NSSFTenant Instance %s has %d other service instances attached" % (service_instance.instance, len(other_service_instances_this_instance)))
    ...
    def delete_instance(self, service_instance, instance):
        ...
        for tag in tags:
            tag.delete()
        #changed name from vm-vrouter-teanant    
    ...
    def save_instance(self, service_instance, instance):
        ...
        super(NSSFTenantPolicy, self).save_instance(instance)
    ...
    def get_image(self, service_instance):
        return service_instance.nssf_vendor.image
    
    def get_flavor(self, service_vendor):
        return service_vendor.nssf_vendor.flavor
